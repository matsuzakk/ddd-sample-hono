...
export const orderShip = async (deps, input) => {
    return deps.db.transaction(async (tx) => {
        ...
        const updatedOrder = order.changeStatus(OrderStatus.create(OrderStatusMap.SHIPPED));
        const history = OrderHistory.create(crypto.randomUUID(), order.id, order.status, OrderStatus.create(OrderStatusMap.SHIPPED));

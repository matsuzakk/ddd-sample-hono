...
ordersRoute.put("/:orderId/cancel", async (c) => {
    ...
    try {
        const result = await orderCancel({ db, createOrderRepository, createOrderHistoryRepository }, { orderId });
...
ordersRoute.put("/:orderId/deliver", async (c) => {
    ...
    const orderId = c.req.param("orderId");
    try {
        const result = await orderDelivered({ db, createOrderRepository, createOrderHistoryRepository }, { orderId });
        return c.json(result);
    }
    catch (e) {
        const msg = String(e);
...
ordersRoute.put("/:orderId/ship", async (c) => {
    ...
    const orderId = c.req.param("orderId");
    try {
        const result = await orderShip({ db, createOrderRepository, createOrderHistoryRepository }, { orderId });
        return c.json(result);
    }
    catch (e) {
        const msg = String(e);
...
ordersRoute.get("/:orderId", async (c) => {
    const db = c.get("db");
    const orderId = c.req.param("orderId");
    const result = await orderDetail({ db, createOrderRepository, createOrderHistoryRepository }, { orderId });
    if (!result.order) {

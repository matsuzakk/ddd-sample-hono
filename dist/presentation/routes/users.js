...
usersRoute.post("/", async (c) => {
    ...
    try {
        const result = await registerUser({ db, createUserRepository }, { name: body.name, email: body.email });
...
usersRoute.get("/:userId/items", async (c) => {
    ...
    const result = await itemSellList({ db, createItemRepository }, { sellerId: userId });
